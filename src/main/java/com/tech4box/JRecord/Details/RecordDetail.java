...
import com.tech4box.JRecord.Common.*;
...
import com.tech4box.JRecord.CsvParser.BasicCsvLineParser;
...
import com.tech4box.JRecord.occursDepending.ODCalculationStandard;
...
//	}
	 ...
	 *
	 *  @see AbstractRecord#getSourceIndex()
	 */
...
//
	...
	public final FieldDetail[] getArrayFields(FieldDetail field, String aname) {
		if (arrays == null) {
			arrays = new HashMap<String, ArrayDtls>();
			ArrayList<ArrayDtls> arrayList = new ArrayList<ArrayDtls>();
			int pos;
			...
			for (ArrayDtls ad : arrayList) {
				ad.fields = ad.fieldList.toArray(new FieldDetail[ad.fieldList.size()]);
				ad.fieldList = null;
			}
		}
		
		ArrayDtls a = arrays.get((field.getGroupName() + aname) .toLowerCase());
