...
//				}
	...
	/**
	 * @see DepthFirstAdapter#inABlankWhenZeroClause(ABlankWhenZeroClause)
	 */
	...
	/**
	 * @see DepthFirstAdapter#inABlankWhenZeroClauseClause(ABlankWhenZeroClauseClause)
	 */
...
//		displayLength = element.displayLength;
	...
	private int setLength(Item element, boolean positive, int displayLength, int assumedDigits, int doubleByteChars) {
	    ...
	    if (element.getChildItems().size() > 0) {
	    	...
	    	if (usage != null && usage != Usage.NONE) {
		    	if (numDef != null) {
	...
	private String getUsage(Item element) {
		...
		if (element.usage != Usage.NONE) {
			...
			while (node.parentItem != null && node.parentItem instanceof Item
			   && (node = (Item) node.parentItem).usage == Usage.NONE) {}
			
			if (node != null && node.usage != null && node.usage != Usage.NONE) {
